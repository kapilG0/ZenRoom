...
use soroban_sdk::{contractimpl, symbol, Env, Symbol, Address, contract, BigInt, Map};
...
impl PolyFund {
    ...
    pub fn vote_request(env: Env, contributor: Address, req_num: u64) {
        ...
        let mut request: Request = env.storage().get(request_key.clone()).unwrap();

        if request.voters.get(&contributor).unwrap_or(false) {
            panic!("You have already voted.");
        }

        request.voters.insert(contributor, true);
